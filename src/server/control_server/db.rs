...
pub async fn init_db_pool() -> anyhow::Result<()> {
    ...
    let _ = sqlx::query("ALTER TABLE devices ADD COLUMN wireguard_public_key TEXT")
        ...
        .await;
    let _ = sqlx::query(
        "ALTER TABLE devices ADD COLUMN vnt_output_subnets TEXT NOT NULL DEFAULT '[]'",
    )
    .execute(&pool)
    .await;
...
pub async fn save_or_update_device(device: &DeviceRecord) -> anyhow::Result<()> {
    ...
    sqlx::query(
        r#"INSERT INTO devices (device_id, network_code, ip, ip_type, client_type, ikev2_password, ikev2_output_subnets, ikev2_input_routes, wireguard_output_subnets, wireguard_input_routes, vnt_output_subnets, wireguard_private_key, wireguard_public_key, device_name, device_version, last_connect_time, tx_bytes, rx_bytes)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
           ON CONFLICT(device_id, network_code) DO UPDATE SET
               ...
               vnt_output_subnets = excluded.vnt_output_subnets,
    ...
    .bind(serde_json::to_string(&device.vnt_output_subnets)?)
...
pub async fn get_device(
    ...
    let row_option = sqlx::query(
        r#"SELECT device_id, network_code, ip, ip_type, client_type, ikev2_password, ikev2_output_subnets, ikev2_input_routes, wireguard_output_subnets, wireguard_input_routes, vnt_output_subnets, wireguard_private_key, wireguard_public_key, device_name, device_version, last_connect_time,
    ...
    match row_option {
        Some(row) => Ok(Some(DeviceRecord {
            ...
            vnt_output_subnets: decode_json_column(&row, "vnt_output_subnets")?,
